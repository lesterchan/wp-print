 ...
 * row at all. Nothing is destroyed by that -- maybe_upgrade() also hangs off
 * admin_init -- which is precisely why it went unnoticed: every site heals the
 * moment somebody opens its dashboard, and a network whose subsites are
 * front-end only never does.
 *
 * @package WP-Print
 */

/**
 * WP_Print::activate() across a network.
 *
 * @group ms-required
 */
class WP_Print_Multisite_Test extends WP_Print_TestCase {
	 ...
	 *
	 * The row is read raw: WP_Print_Options answers with the defaults whether
	 * the row exists or not, which is exactly the difference under test.
	 *
	 * @return void
	 */
	public function test_network_activation_seeds_every_site() {
		...
		WP_Print::activate( true );

		foreach ( $site_ids as $site_id ) {
			switch_to_blog( $site_id );

			$this->assertIsArray(
				get_option( WP_Print_Options::OPTION ),
