	...
	function print_default_templates(template) {
		...
		switch(template) {
			case 'html':
				...
				break;
			case 'disclaimer':
				default_template = '<?php echo esc_js( sprintf( __( 'Copyright &copy; %1$s %2$s. All rights reserved.', 'wp-print' ), date( 'Y' ), get_option( 'blogname' ) ) ); ?>';
				break;
		}
	...
	<table class="form-table">
		...
		<tr>
			...
			<td>
				...
				<div id="print_style_custom" style="display: 
				<?php
				if ( intval( $print_options['print_style'] ) == 4 ) {
					echo 'block';
				} else {
					echo 'none'; }
				?>
